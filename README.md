...
This repository will contain that code that has been validated, or it is currently under development. As of current moment the "main" branch contains the necessary code to perform P/L 1 basic operation. This is the taking of an image through telecommand and its posterior transmission through the communications subsystem.

## Repository Organization
<a name="organization"></a>
In this repository contains the following repositories: `CommonResources`, `Core`, . They are organized as follows:

The following folders are generated automatically when generating the project in the 

- **CommonResources**: In here the general resource libraries can be found. In specially the current code to flash the software into the development boards.

- **Core**: Autogenerated directory that contains 3 folders: `Inc`, `Src`, `Startup`.
...
- **Drivers**: Autogenerated folder that contains the **HAL** drivers and **CMSIS** files.

- **Middlewares**: This folder contains the FreeRTOS libraries.

- **Semtech**: This folder contains the drivers to control the Communications Xip.

- **Subsystems**: On the other hand, each subsystem has its own directory. They can be found in `Subsystems` The structure of the following directories consist of: `name_of_the_subsystem.c` which will be the main file of the directory, `inc` where all the headers are found, and other libraries used by the subsystem.
...
- **`pocat-sw.ioc`**: This is the pin configuration file.

**Note**: when compiling the code for the first time, the `Debug` folder will be autogenerated. Please do not upload it to the repository, it is ignored.
