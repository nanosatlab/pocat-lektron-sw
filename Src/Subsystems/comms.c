 ...
 */
 ...
 uint8_t RxData_Comms[48];        //Tx and Rx decoded
 uint8_t TxPacket[48];
...
//  TimerHandle_t xTimerBeacon;
               ...
               if (Beacon_Flag)
         {
             ...
             case STDBY:
               ...
               case (UPLOAD_COMMS_CONFIG):
                     Radio.Standby();
                 SX1262TLCConfig(RxData_Comms);
                     COMMS_State=TX;
                     Beacon_Flag=1;
                     break;
               case(COMMS_UPLOAD_PARAMS):
           Radio.Standby();
           COMMSTLCConfig(RxData_Comms);
           COMMS_State=TX;
                     Beacon_Flag=1;
                 break;
 ...
 void OnRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
 {
   ...
   memset(RxData_Comms,0,sizeof(RxData_Comms));
     ...
     memcpy(RxData_Comms,RxPacket,size);
     ...
     if (RxData_Comms[0]==0xC8 && RxData_Comms[1]==0x9D)
     {
         TLCReceived_Flag=1;
         COMMS_State=RX;
     }
