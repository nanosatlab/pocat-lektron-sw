...
void OBC_Init() {
  uint8_t currentState; // OBC state : INIT, NOMINAL, CONTINGENCY, SUNSAFE OR
                        // SURVIVAL.
  uint8_t *deploy =
      0; // Boolean for deployment. True if deployed. False otherwise.

  xTaskNotify(ADCS_Handle, DETUMBLING_NOTI,
              eSetBits); // Stabilize the satellite.
                         // OBC TASK -> ADCS TASK.

  xTaskNotify(
      EPS_Handle, EPS_BATTERY_NOTI,
      eSetBits); // Read battery in order to know if we have enough capacity for
                 // antenna deployment. EPS task updates the current state
                 // according to the battery capacity. OBC TASK -> EPS TASK.

  Read_Flash(CURRENT_STATE_ADDR, &currentState,
             1); // Read the current state of the flash.

  if (currentState ==
      _NOMINAL) // Deploy antennas only if the next state is NOMINAL.
  {
    xTaskNotify(COMMS_Handle, ANTENNA_DEPLOYMENT_NOTI,
                eSetBits); // Deploy COMMS antenna.
                           // OBC TASK -> COMMS TASK.

    // xTaskNotify(RFI_Handle, ANTENNA_DEPLOYMENT_NOTI, eSetBits); // Deploy RF
    // antenna. OBC TASK -> RFI TASK.
    *deploy = 1;
  }

  Send_to_WFQueue(deploy, 1, ANTENNA_DEPLOYED_ADDR,
                  OBCsender); // Save to the memory flash the current deployment
                              // state OBC TASK -> FLASH TASK

  xTaskNotify(ADCS_Handle, CHECKROTATE_NOTI,
              eSetBits); // Poll lateral temperature sensors and determine if
                         // rotation is needed OBC TASK -> ADCS TASK
...
void OBC_Nominal() {
  ...
  CPUFreq = HAL_RCC_GetHCLKFreq(); // Read the current CPU frequency stored
                                   // within the RCC registers of the chip.

  if (CPUFreq != 80000000) // If different from the frequency at which the
                           // NOMINAL state has to run.
  {
    ...
    SysTick_Config(SystemCoreClock /
                   1000);  // Initialize the Systick and restart
  ...
  if (!deploy) {
    ...
    deploy = 1;
    Send_to_WFQueue(&deploy, 1, ANTENNA_DEPLOYED_ADDR,
                    OBCsender); // Save to the memory that we have already
                                // deployed the antennas.
  }
  ...
  /*
   * telecommand_process is a boolean that determines whether or not a
   * telecommand is on its way to be processed. If its the case, the software
   * will not check if it has something to do (notifications or packets) because
   * we already know. Then, the 15 s will be given directly.
   */
  ...
  for (;;) {
    ...
    if (!telecommand_process) {
      ...
      EventBits = xEventGroupWaitBits(
          xEventGroup, COMMS_RXIRQFlag_EVENT | COMMS_RXNOTI_EVENT, 0, false,
          pdMS_TO_TICKS(500));
    }

    if (((EventBits & COMMS_RXIRQFlag_EVENT) == COMMS_RXIRQFlag_EVENT) ||
        ((EventBits & COMMS_RXNOTI_EVENT) == COMMS_RXNOTI_EVENT)) {
      /*
      ...
      EventBits = xEventGroupWaitBits(
          xEventGroup, COMMS_TELECOMMAND_EVENT | COMMS_WRONGPACKET_EVENT, 0,
      ...
      else if (((EventBits & COMMS_TELECOMMAND_EVENT) !=
                COMMS_TELECOMMAND_EVENT) &&
               ((EventBits & COMMS_WRONGPACKET_EVENT) !=
    ...
    xTaskNotify(
        EPS_Handle, EPS_HEATER_NOTI,
        eSetBits); // Poll temperature sensor of the battery and determine if
                   // heater activation is needed. OBC TASK -> EPS TASK.

    xTaskNotify(EPS_Handle, EPS_BATTERY_NOTI,
                eSetBits); // Read battery.
...
void OBC_Contingency() {
  ...
  xTaskNotify(
      COMMS_Handle, CONTINGENCY_NOTI,
      eSetBits); // Notify COMMS that we have entered the CONTINGENCY state
  xTaskNotify(
      ADCS_Handle, CONTINGENCY_NOTI,
      eSetBits); // Notify ADCS that we have entered the CONTINGENCY state
...
void OBC_Sunsafe() {
  ...
  xTaskNotify(COMMS_Handle, SUNSAFE_NOTI,
              eSetBits); // Notify COMMS that we have entered the SUNSAFE state.
  xTaskNotify(ADCS_Handle, SUNSAFE_NOTI,
              eSetBits); // Notify ADCS that we have entered the SUNSAFE state.
  ...
  TIM7->PSC = 40000;     // 16-bit Prescaler : TIM7 frequency = CPU_F/PSC
                         // = 8.000.000/40.000 = 200 Hz.
  TIM7->ARR = 2000; // 16-bit Auto Reload Register : ARR = Sleep_time * TIM7_F =
                    // 10*200=2000.
  ...
  for (;;) {
    ...
    SysTick->CTRL &=
        ~SysTick_CTRL_TICKINT_Msk; // Suspend the scheduler (systick timer)
                                   // interrupt to not wake up the MCU
    HAL_SuspendTick(); // Suspend the tim6 interrupts to not wake up the MCU

    HAL_PWR_EnterSLEEPMode(
        PWR_MAINREGULATOR_ON,
        PWR_SLEEPENTRY_WFI); // Enter in SLEEP MODE with the WFI (Wait For
    ...
    SysTick->CTRL |= SysTick_CTRL_TICKINT_Msk; // Enable the scheduler (systick
                                               // timer) interrupt
...
void OBC_Survival() {
  ...
  /****TIMER 7 CONFIGURATION FOR A 10s SLEEP*****/
  TIM7->SR &= ~(1 << 0); // Clear update interrupt flag
  TIM7->PSC = 20000;     // 16-bit Prescaler : TIM2 frequency = CPU_F/PSC
                         // = 2.000.000/20.000 = 100 Hz.
  TIM7->ARR = 1000; // 16-bit Auto Reload Register : ARR = Sleep_time * TIM7_F =
                    // 10*100=1000.
